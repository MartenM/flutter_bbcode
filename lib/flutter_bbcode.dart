...
class BBCodeText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if(error != null || stackTrace != null) {
      ...
      return RichText(text: const TextSpan(text: "An error occured attempting to parse the BBCode."));
    }
...
List<InlineSpan> parseBBCode(
  ...
  final nodes = bbob.parse(
      data,
      validTags: parsers == null ? allTags.map((e) => e.tag).toSet() : parsers.map((e) => e.tag).toSet(),
      onError: (parseError) {
  ...
  FlutterRenderer renderer = FlutterRenderer(
      defaultStyle: defaultStyle ?? const TextStyle(color: Colors.black, fontSize: 14),
